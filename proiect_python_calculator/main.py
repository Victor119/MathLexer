...
def inlocuieste_cu_minus(sir):
    ...
    # trebuie sa stergem semnul plus din sir daca e pe prima pozitie
...
def calculam_lst_fara_paranteze(sir):
    ...
    if "rad" not in formula and "log" not in formula and "sin" not in formula and "cos" not in formula:
        ...
        #trebuie sa extragem din formula 3 incepand de la rad si sa inlocuim cu valoarea adecvata
        n = len(formula)
        ok_2 = 0
        i = 0
...
#sir = "2 * (3 + 5 * ( 6 / (7 + 1)- 4)) - rad(9+5) / (3 + 5 ^^ 2 / 8)"
...
#de rezolvat minus in penultimul exemplu, nu da corect


#stergem toate spatiile goale (whitespaces-urile) din sir
sir = sir.replace(" ", "")

#print("rezultat_cautat", calculam_lst_fara_paranteze(sir))
